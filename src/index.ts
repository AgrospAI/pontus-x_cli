...
program
    .command("self-description <did> <sdurl>")
    .description(
...
program
    .command("change-price <did> <newPrice>")
    .description(
    ...
    .action(async (did, newPrice) => {
        ...
        if (
            readlineSync.keyInYNStrict(
                `Set the price to ${newPriceNumber.toString()} for asset ${did}? `
            )
        ) {
            try {
                const aquariusAsset =
                ...
                const tx = await connection.nautilus.setServicePrice(
                    aquariusAsset,
                    serviceId,
                    newPriceNumber.toString()
                );
                console.log(
...
program
    .command("edit-algo <did> <image> <tag> <checksum>")
...
program
    .command("edit-trusted-algos <did> <algos...>")
    .description(
...
program
    ...
    .action(async (scriptFolder, options) => {
        const connection = await Connection.connect();
        console.log(
            `Publishing asset ${scriptFolder} in provider ${options.provider} from wallet ${connection.wallet.address}`
        );
        try {
            await invokeFunctionFromFile(
                `${scriptFolder}/index.ts`,
                "publish",
                scriptFolder,
                connection,
                options.provider,
...
program
    ...
    .action(async (algo: string, datasets: string[]) => {
        ...
        if (
            ...
            try {
                const firstDatasetAsset =
                    await connection.nautilus.getAquariusAsset(datasets[0]);
...
program
    ...
    .action(async (jobId: string, options) => {
        ...
        try {
            ...
            if (computeResultUrl)
                console.log(
                    `Compute results available from: ${computeResultUrl}\n`
                );
            else console.log(`No results available yet\n`);
        } catch (e) {
            console.error(`Error starting compute: ${e}`);
        }
        process.exit(0);
...
program
    ...
    .requiredOption(
        "-d, --domain <url>",
        "URL where the DID-Web document will be hosted"
    )
    .requiredOption(
        "-c, --certificate <certificate-chain.crt>",
...
program
    ...
    .requiredOption(
        "--vp <participant.vp.json>",
        "Path to the participant Verifiable Presentation file"
    )
    .action(async (options) => {
        try {
...
program
    ...
    .description(
        "Install the autocompletion for the Pontus-X CLI using tabtab"
    );

program
    .command("uninstall-completion")
    .description(
        "Uninstall the autocompletion for the Pontus-X CLI using tabtab"
    );
...
const completion = (env: tabtab.TabtabEnv) => {
    ...
    if (commands.includes(env.prev)) {
        return tabtab.log(["--help"]);
    }

    return tabtab.log([...options, ...commands]);
};
