...
export default class GenerateDidWeb extends Command {
  static description = 'Generate a did.json to set up a DID-Web source to publish Gaia-X complaint credentials'
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const didJson = await createDidDocument(flags.domain, basename(flags.certificate), x509)
