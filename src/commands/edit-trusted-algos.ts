...
export default class EditTrustedAlgos extends Command {
  ...
  async run(): Promise<void> {
    ...
    for (const asset of assets) {
      console.log(`Editing trusted algorithms for asset ${chalk.gray(asset.did)} (${chalk.magenta(asset.name)})...`)
      try {
        ...
        if (flags.public) {
          ...
          serviceBuilder.addTrustedAlgorithms(algorithms.map((algo: AssetInfo) => ({ did: algo.did })))
        } else {
          // Private
          serviceBuilder.setAllAlgorithmsUntrusted()
        }
...
async function askForDatasets(loginInfos: LoginInfo[], chainId: number): Promise<AssetInfo[]> {
...
async function askForAlgorithms(loginInfos: LoginInfo[], chainId: number): Promise<AssetInfo[]> {
