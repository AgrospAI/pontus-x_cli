  ...
  async run(): Promise<void> {
    ...
    if (
      ...
      readlineSync.keyInYNStrict(
        `Update the asset additional metadata for asset ${did} to that in file "${metadatafile}? `,
      )
    ) {
      try {
        ...
        this.log(
          `New additional metadata: ${JSON.stringify(additionalMetadata, null, 2)}`,
        );
