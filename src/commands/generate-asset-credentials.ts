...
export default class GenerateAssetCredentials extends Command {
  ...
  static description = 'Generate the Gaia-X credentials for the input DID asset, including its verifiable presentation'
  ...
  async run(): Promise<void> {
    ...
    try {
      const password =
        flags.password ??
        readlineSync.question(`Enter the password for your private key file ${flags.certificate}: `, {
          hideEchoBack: true,
        })
      await generateCredentials(flags.participant, flags.didjson, flags.certificate, password, ddo)
    } catch (error) {
