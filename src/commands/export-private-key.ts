...
export default class ExportPrivateKey extends Command {
  ...
  static flags = {
    filepath: Flags.string({
      char: "f",
      description:
        "File path to save the encrypted private key (default is <address>.json)",
      required: false,
    }),
  ...
  async run(): Promise<void> {
    ...
    if (flags.privateKey?.startsWith("0x")) {
    ...
    console.log(
      `Generating encrypted file to store your private key, which corresponds to you account ${address}`,
    );
