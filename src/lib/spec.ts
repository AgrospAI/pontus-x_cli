...
function getFiles(rootPath: string, preference: string[] = ['spec.yaml', 'spec.json', 'index.ts']): string[] {
...
export function getFilesFromPaths(paths: string[], preference?: string[]): string[] {
  return paths.flatMap(path => getFiles(path, preference))
}

export async function getEnvironmentConfig(file: string, config: PublishConfig, manifest: Manifest) {
  ...
  if (owner && !privateKey) {
    ...
    try {
      privateKey = await readPrivateKey(account?.privateKeyPath || '', password)
    } catch (error) {
...
function getPricing(
  ...
  const pricingFn = pricingMap[currency as keyof typeof pricingMap] ?? pricingMap.OCEAN

  return pricingFn(amount)
}

export async function publishFromFile(filePath: string, providerUrl: string, connection: Connection, dryRun: boolean) {
  ...
  if (config.metadata.type === 'algorithm' && config.metadata.algorithm) {
    ...
    assetBuilder.setAlgorithm({
      container: {
        ...config.metadata.algorithm.container,
        consumerParameters: undefined,
      },
      language: config.metadata.algorithm.language,
      version: config.metadata.algorithm.version,
      ...(consumerParameters.length > 0 && { consumerParameters }),
    })
  }
