...
export function getPrivateKeyForOwner(loginInfos: LoginInfo[], ownerAddress: string): string {
...
export async function manifestToLoginInfos(manifest: Manifest): Promise<LoginInfo[]> {
  return Promise.all(
    manifest.accounts.map(async account => {
      try {
        const password = process.env[account.passwordEnvKey || ''] || ''
        const privateKey = await readPrivateKey(account.privateKeyPath, password)
...
export async function getLoginInfos(manifestPath?: string): Promise<LoginInfo[]> {
  let loginInfos: LoginInfo[] = []
  if (manifestPath) {
    console.log(chalk.green(`Using manifest file for authentication ${chalk.gray(`(path: ${manifestPath})\n`)}`))
    const manifest = readManifest(manifestPath)
    loginInfos = (await manifestToLoginInfos(manifest))
      .filter(info => info.ownerName !== '-')
      .sort((a, b) => a.ownerName.localeCompare(b.ownerName))
  } else {
    console.log(chalk.green(`Using environment variables for authentication ${chalk.gray('(env: PRIVATE_KEY)')}\n`))
...
export function getEnvNetwork(): Network | null {
  ...
  const network = Object.keys(NETWORK_CONFIGS).find(network => network === envNetwork)
