...
export class Connection {
  ...
  public static async connect(envOverrides?: Record<string, string>): Promise<Connection> {
    const env = {...process.env, ...envOverrides}

    if (!env.NETWORK) {
      throw new Error(`Set your network in the .env file. Supported networks are ${Object.values(Network).join(', ')}.`)
    }

    const selectedEnvNetwork = env.NETWORK.toUpperCase()
    if (!(selectedEnvNetwork in Network)) {
      throw new Error(
        `Invalid network selection: ${selectedEnvNetwork}. Supported networks are ${Object.values(Network).join(
          ', ',
        )}.`,
      )
    }
