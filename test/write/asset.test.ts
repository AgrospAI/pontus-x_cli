...
describe.sequential('Asset Commands', () => {
  ...
  test('edit-trusted-algos command updates trusted algorithms', async () => {
    await EditTrustedAlgos.run(['-d', assets.dataset2Did, '-a', assets.algorithm1Did, assets.algorithm2Did, '-y'])
  }, 30_000)
  ...
  test(
    'compute job works',
    withLogin(async () => {
      const computeJob = await Compute.run([assets.algorithm1Did, '-d', assets.dataset2Did, '-y'])
  ...
  test(
    ...
    withLogin(async () => {
      ...
      await EditAssetAllowed.run(['-u', allowedAddress, '-d', assets.dataset1Did, '-y'])
    }),
    30_000,
  )
  ...
  test(
    ...
    withLogin(async () => {
      const allowedAddress = '0xAbC1230000000000000000000000000000dEf1'
      await EditAssetAllowed.run(['-u', allowedAddress, '-d', assets.algorithm1Did, '-y'])
    }),
    30_000,
  )
  ...
  test(
    ...
    withLogin(async () => {
      const allowedAddress = '0xAbC1230000000000000000000000000000dEf1'
      await expect(
        EditAssetAllowed.run(['-u', allowedAddress, '-d', assets.dataset1Did, '--public', '-y']),
      ).rejects.toThrow()
    }),
    30_000,
  )
  ...
  test(
    ...
    withLogin(async () => {
      await expect(EditAssetAllowed.run(['-d', assets.dataset1Did, '-y'])).rejects.toThrow()
    }),
    30_000,
  )
})
